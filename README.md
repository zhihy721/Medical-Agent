...
```env
AMAP_API_KEY=          # 高德地图 Web 服务 key，配置后支持定位就近检索与按城市检索真实医院 POI
HOSPITAL_DATA_URL=     # 或静态 JSON 数据源，优先级高于高德（仅城市级，不支持定位）
```

注意：这两个手填键不在 web 配置页展示/管理范围内，但配置页保存时会原样保留（序列化保留非默认键）。

浏览器定位：通过 `http://localhost`（或 127.0.0.1）访问时，页面加载会请求一次定位授权；授权后高风险升级时按真实位置检索 10km 内最近 5 家医院（按直线距离排序，10km 无结果自动扩大到 20km 并在回复中标注），拒绝或不可用时静默回退城市名查询。坐标仅会话内用于医院检索，不写入长期画像；局域网 IP 的 http 访问无法使用 Geolocation API。
...
基本信息与长期记忆：侧栏“我的信息”卡片可手填年龄/性别/既往史/过敏史/用药史（病史逗号分隔），保存后写入长期画像并立即同步当前会话，风险评估与追问决策即时生效；新会话会自动用画像预填年龄/病史等空字段（恢复旧会话时已有值不覆盖）。手填属于用户权威编辑，不计入槽位历史、不触发矛盾检测；“清除长期记忆”按钮仅删除画像文件，不影响本次对话。
...
- 多轮问诊状态管理
- 症状抽取、缺失字段追问、风险等级展示
- 长期用户画像和短期会话状态分离，JSON 文件持久化，重启不丢失；侧栏可手填基本信息（年龄/性别/病史/过敏史/用药史），新会话自动预填画像，支持一键清除长期记忆
...
- MCP 接入：stdio 桥接 + 工具适配与降级，试点服务 hospital_locator（附近医院：定位就近检索按距离取最近 5 家，数据源可切高德地图/静态端点/演示）
- 风险规则外置（risk_rules.json）：症状组合、既往史、高龄等条件命中即升级
- 红旗否认豁免：用户明确否认的红旗不误报、不误升级
- 位置来源链：浏览器定位坐标（优先） > 自由文本抽取城市 > 环境变量 MEDICAL_AGENT_LOCATION，驱动就近医院查询
